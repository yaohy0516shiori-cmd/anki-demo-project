...
from backend.app.deps import get_deck_service
...
@router.post("")
def create_deck(payload: DeckCreate, deck_service=Depends(get_deck_service)):
    try:
...
@router.get("")
def list_decks(deck_service=Depends(get_deck_service)):
    return [deck_to_dict(deck) for deck in deck_service.get_all_decks()]


@router.get("/{deck_id}")
def get_deck(deck_id: int, deck_service=Depends(get_deck_service)):
    try:
...
def update_deck(deck_id: int, payload: DeckUpdate, deck_service=Depends(get_deck_service)):
    try:
        current = deck_service.get_deck(deck_id)
        ...
        current.touch()
        saved = deck_service.update_deck(current)
        return deck_to_dict(saved)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def delete_deck(deck_id: int, hard: bool = False, deck_service=Depends(get_deck_service)):
    try:
        if hard:
